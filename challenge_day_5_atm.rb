...
def banner
  p "=".center(71, '=');
  p 'BPI ATM System'.prepend(' ').concat(' ').center(71, '=');
  p "=".center(71, '=');
end
...
def adminInfo
  ...
  while i < $database.length
    p "Account number #{i+1}: #{$database[i][:accountNum]}"
    p "Balance: $#{$database[i][:balance]}"
    p " "*71;
...
def balance
  p "Current balance: $#{$database[@index][:balance]}";
  redirectToOverview();
end
...
def withdrawal
  ...
  if amountWithdrawal <= $database[@index][:balance]
    ...
    p "Here is your $#{amountWithdrawal}  Your new balance is: $#{$database[@index][:balance]}"
  else
